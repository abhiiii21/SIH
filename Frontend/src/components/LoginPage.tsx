...
export const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If error is 401 unauthorized from live backend, show message, otherwise allow offline demo login
      if (err?.response?.status === 401) {
        setAuthError(err?.response?.data?.detail || "Invalid credentials");
      } else {
        // Allow seamless demo pass
        if (onLogin) {
          onLogin(email || "commander.patil@coastguard.gov.in");
        }
      }
  ...
  return (
    <div className="relative w-screen h-screen overflow-x-hidden overflow-y-auto lg:overflow-hidden select-none bg-[#031525] font-sans">
      {/* 1. FULL-BLEED REALISTIC OCEAN BACKGROUND (Edge-to-Edge) */}
      <div
      ...
      <div className="absolute top-0 right-0 w-[480px] h-[380px] pointer-events-none overflow-hidden opacity-70 hidden md:block">
        ...
        >
          ...
          <path
            ...
            strokeDasharray="3 4"
          />
          {/* Satellite Icon (detailed illustration) */}
      ...
      {/* 3. TOP BAR: HEADER (Left) & NAV (Right) */}
      <header className="relative z-20 w-full px-6 sm:px-10 lg:px-14 pt-6 pb-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3.5">
          {/* Circular Blue Wave / Infinity Logo */}
        ...
        {/* Top-Right: Nav Items ("Detect • Analyze • Protect") */}
      ...
      {/* 4. MAIN CONTENT VIEWPORT: Split Screen (Hero on Left, Login Card on Right) */}
      <main className="relative z-10 w-full min-h-[calc(100vh-8.5rem)] px-6 sm:px-10 lg:px-14 flex flex-col lg:flex-row items-center justify-between py-6 lg:py-0">
        ...
        {/* LEFT ZONE: HERO TEXT & 5 FEATURE ICONS */}
        <div className="w-full lg:w-[54%] max-w-[660px] flex flex-col justify-center py-4 lg:py-8">
          ...
          {/* Feature Icon Row: 5 Circular Light-Blue Items */}
        ...
        {/* RIGHT ZONE: FLOATING FROSTED-GLASS LOGIN CARD */}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between pt-1 text-xs">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-[#0B2545]/80 font-medium">
                    Remember me
                  </span>
                </label>
      ...
      {/* 5. PAGE FOOTER (Bottom, Overlaid on Ocean Image) */}
