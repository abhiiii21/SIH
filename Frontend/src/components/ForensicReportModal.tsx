...
export const ForensicReportModal: React.FC<ForensicReportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 select-none font-body antialiased">
      <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col rounded border border-zinc-700 bg-zinc-950 shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900 px-6 py-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="heading-section text-sm font-bold tracking-wider text-zinc-100 uppercase">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="flex items-center space-x-1.5 rounded border border-zinc-700 bg-zinc-800 hover:bg-zinc-700 px-3 py-1.5 text-xs font-semibold font-body text-zinc-200 transition touch-target"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 text-xs font-body">
          {/* Metadata Grid */}
          <div className="grid grid-cols-3 gap-3 rounded border border-zinc-800 bg-zinc-900/60 p-3.5">
            <div>
              <span className="text-[10px] text-zinc-400 uppercase font-semibold">Sector</span>
              <div className="font-bold text-zinc-200 mt-0.5 font-body">{activeIncident}</div>
            </div>
            <div>
              <span className="text-[10px] text-zinc-400 uppercase font-semibold">Acquisition UTC</span>
              <div className="font-bold text-zinc-200 mt-0.5 font-mono">{nowUtc}</div>
            </div>
            <div>
              <span className="text-[10px] text-zinc-400 uppercase font-semibold">Legal Standard</span>
              <div className="font-bold text-zinc-200 mt-0.5 font-body">MARPOL Annex I / UNCLOS</div>
            </div>
          </div>

          {/* Section 1: SAR Oil Slick Evidence */}
          <div className="space-y-2">
            <h3 className="heading-section text-xs font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-1.5 border-b border-zinc-800 pb-1">
            ...
            <div className="grid grid-cols-4 gap-2 text-[11px]">
              <div className="rounded bg-zinc-900 p-2.5 border border-zinc-800">
                ...
                <div className="text-sm font-bold text-red-400 mt-0.5 font-body">
                  {spillData?.summary.total_area_km2.toFixed(2)} km²
                </div>
              </div>
              <div className="rounded bg-zinc-900 p-2.5 border border-zinc-800">
                <span className="text-zinc-400">Est. Volume</span>
                <div className="text-sm font-bold text-amber-400 mt-0.5 font-body">
          ...
          <div className="space-y-2">
            ...
            <div className="rounded bg-zinc-900 p-3 border border-zinc-800 space-y-2 text-[11px]">
              <div className="flex items-center justify-between text-zinc-300">
                ...
                <span className="text-zinc-100 font-bold font-mono">V_drift = V_current + 0.035 × V_wind</span>
              </div>
              <div className="grid grid-cols-3 gap-2 pt-1 border-t border-zinc-800">
                <div>
                  <span className="text-zinc-400">Net Drift Speed:</span>
                  <div className="text-zinc-200 font-bold font-mono">{driftData?.net_drift_speed_knots} kts ({driftData?.net_drift_speed_ms} m/s)</div>
                </div>
                <div>
                  <span className="text-zinc-400">Net Drift Heading:</span>
                  <div className="text-zinc-200 font-bold font-mono">{driftData?.net_drift_heading_deg}°</div>
                </div>
                <div>
                  <span className="text-zinc-400">Origin Window BBox:</span>
                  <div className="text-zinc-200 font-bold font-mono truncate">[{driftData?.origin_window_bbox.map(n => n.toFixed(3)).join(", ")}]</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="overflow-x-auto rounded border border-zinc-800 bg-zinc-900">
              <table className="w-full text-left text-[11px] font-body">
                <thead className="bg-zinc-950 text-zinc-400 border-b border-zinc-800 uppercase tracking-wider text-[10px] font-semibold">
                  <tr>
                ...
                <tbody className="divide-y divide-zinc-800 text-zinc-200">
                  {aisData?.ranked_suspects.map((v) => (
                    <tr key={v.mmsi} className={v.attribution_rank === 1 ? "bg-red-950/20" : ""}>
                      <td className="p-2 font-bold font-mono text-red-400">#{v.attribution_rank}</td>
                      <td className="p-2 font-bold font-body">{v.vessel_name}</td>
                      <td className="p-2 text-zinc-400 font-mono">{v.imo} ({v.flag})</td>
                      <td className="p-2 text-zinc-400 font-body">{v.vessel_type}</td>
                      <td className="p-2 font-semibold font-mono text-red-400">{v.closest_approach_km} km</td>
                      <td className="p-2 font-semibold font-mono text-amber-300">{v.min_sog_knots} kts</td>
                      <td className="p-2 font-semibold font-mono text-red-400">{v.max_transponder_gap_mins}m</td>
                      <td className="p-2 text-right font-bold font-mono text-red-400">{v.liability_probability_pct}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {primarySuspect && (
            <div className="rounded border border-red-900/60 bg-red-950/20 p-3.5 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-red-400 uppercase font-display tracking-wide">
                  Statement of Primary Liability &amp; Anomaly Evidence
                </span>
                <span className="text-xs font-bold font-mono text-red-300 bg-red-950 px-2 py-0.5 rounded border border-red-800">
                  {primarySuspect.liability_probability_pct}% CONFIDENCE
                </span>
              </div>
              <p className="text-[11px] leading-relaxed text-zinc-300 font-body">
                Vessel <strong className="text-zinc-100 font-semibold">{primarySuspect.vessel_name}</strong> ({primarySuspect.vessel_type}, Flag: {primarySuspect.flag}, IMO: <span className="font-mono">{primarySuspect.imo}</span>) traversed within <strong className="text-red-400 font-mono">{primarySuspect.closest_approach_km} km</strong> of the hydrodynamic spill origin. The vessel exhibited kinematic anomalies characteristic of illegal bilge or slop tank discharge: reduction of speed to <strong className="text-red-400 font-mono">{primarySuspect.min_sog_knots} knots</strong> and an AIS transponder blackout of <strong className="text-red-400 font-mono">{primarySuspect.max_transponder_gap_mins} minutes</strong> within the origin surveillance box.
              </p>
            </div>
          )}
        </div>
        ...
        {/* Modal Footer */}
        <div className="flex items-center justify-between border-t border-zinc-800 bg-zinc-900 px-6 py-3 text-[11px] text-zinc-400 font-body">
          <span className="tracking-wider uppercase text-[10px]">SAHAYYA FORENSIC ENGINE • DIGITAL CHAIN OF CUSTODY VERIFIED</span>
          <button
            onClick={onClose}
            className="rounded border border-zinc-700 bg-zinc-800 hover:bg-zinc-700 px-4 py-1.5 text-xs font-semibold font-body text-zinc-200 transition touch-target"
          >
            CLOSE DOSSIER
          </button>
        </div>
